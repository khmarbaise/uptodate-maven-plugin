...
/**
 * Will check the parent of your project and fail the build if the parent is not
 * the newest available version of the parent.
 * 
...
public class ParentMojo extends AbstractUpToDateMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		if (parentProject != null && getMavenProject().isExecutionRoot()) {
			...
			try {
				String id = parentProject.getGroupId() + ":" + parentProject.getArtifactId() + ":"
						+ parentProject.getVersion();
				List<Version> versions = getNewerVersionsOfArtifact(parentProject.getGroupId(),
						parentProject.getArtifactId(), parentProject.getVersion(), null, "pom");
				getLog().debug(id + " existing versions:" + versions.size() + " version:" + join(versions));

				if (versions.size() > 1) {
					Version newest = versions.get(versions.size() - 1);
					throw new MojoExecutionException(
							"There is a more up-to-date version ( " + newest + " ) of the parent available.");
				}
